   ...
   <body>
      ...
      @foreach($data as $d)
       <tr class='data-item' data-id="{{$d->id}}">
        ...
        <td>
         <form action="{{route('order_partial_update',['id'=>$d->id])}}" class="partial-update-form" method='POST'>
          @csrf
          @method('PUT')
          <select name="stateId" id="" class='form-control sensor'>
           @foreach($states as $s)
            @php
             $selected=$s->id==$d->stateId?"selected='selected'":null;
            @endphp
        ...
        <td>
         <a href="{{route('order_details',['id'=>$d->id])}}" class="btn">Details</a>
        </td>
       </tr>
       @php
        $index++;
